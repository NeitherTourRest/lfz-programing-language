...
- **重绑定的错误类与可变性边界（v1 补钉，规范性；见 §8.1 `TypeError` 行）**：**仅经显式 `let` 声明的绑定不可重绑定**——对其重绑定（`a = v` 或复合赋值 `a += v` 等，且 lvalue 基为**裸 `IDENT`**）→ **`TypeError`**，消息 **`不能重新赋值 let 变量 '{name}'；let 只锁重绑定，不锁内容`**（`{name}` = 被重绑定的名字），`span` = 赋值目标变量名首字符。**`var` 绑定、函数 / λ 形参、`for` 循环变量、`fn` 名、`struct` 模板名一律可变**（非 `let`；与现有实现一致：`src/ast.rs:90` `let→mutable:false / var→mutable:true`，`src/evaluator.rs:1110` 形参、`:596` `for` 变量、`:612/:625` `fn`/`struct` 名均 `mutable:true`）。**容器原地修改 `a[i] = v` / `s.k = v` 即使 `a`/`s` 为 `let` 亦合法**（A1）——本规则**只**约束对**裸变量名**的重绑定。
...
| 重绑定 `let` 变量（v1 补钉） | `不能重新赋值 let 变量 '{name}'；let 只锁重绑定，不锁内容` |
...
**traceback 折叠（v1 补钉，规范性；深栈保护）**：渲染运行期 traceback 时，设帧总数 `T`：

- `T ≤ 40` → **原样逐帧输出**（浅栈行为**逐字节不变**）。
- `T > 40` → **首 10 帧**（最外层）→ **一行省略行** → **尾 30 帧**（最内层）→ 末行。
- **省略行格式（逐字符）**：`  ... 省略 {N} 帧 ...`（前缀 **2 空格**，与 `File` 行缩进对齐；`N = T − 40`，十进制）。
- **仅作用于人类可读 stderr 渲染**；`--json` 的 `traceback` 数组**不折叠**（§8.4）；退出码不变（仍 `2`）。
- 常量：`TRACEBACK_HEAD = 10`、`TRACEBACK_TAIL = 30`、阈值 `= 40`（实现落点见 [interface-contract.md](./interface-contract.md) §10.3；由 **tooling-dev** 在 CLI `render_error` 落地，**不**改 `LfzError` / `TracedRun`）。
...
- **`traceback` 数组完整、不折叠（v1 补钉）**：机器可读的 `traceback` 恒为**自最外层到最内层完整帧表**（**不**受 §8.2 人类输出折叠影响）；深递归时元素数可达递归上限量级（~10000），渲染方自行折叠。schema 与元素 `{file,line,func}` **不变**。
