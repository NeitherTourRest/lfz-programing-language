# LFZ 语言 v0.3 设计草案（DRAFT · 修订版）

> 作者: language-architect ｜ 日期: 2026-09-23 ｜ 状态: **草案，非冻结 spec**
> 依据: `.opencode/team/KICKOFF.md`、`.opencode/team/DECISIONS.md`（D-007 v1 范围、D-011 实现语言 = Rust 与 float 纳入、D-012 v0.2 语法修订）、`.opencode/team/DRAFT-LFZ-v0.2.md`、**用户本轮新指令（文件前导 `#42` / Python 风格错误模型 / 可移植性）**。
> 本文件是 v0.2 草案的**修订版**：只重写受用户新指令影响的部分；未触及处沿用 v0.2。
...
1. **每个 `.lfz` 文件的第一行必须恰好是 `#42`**（其后紧跟一个行终止符）——这是「宇宙的答案」仪式。缺失即抛 `CosmosAnswerError: 你忘记了宇宙的答案`。**REPL / stdin / `lfz run -e` 豁免**。
...
| # | 用户指令 | v0.2 | v0.3 采用 |
|---|---|---|---|
| A | **文件前导 `#42`** | 无 | **新增**：`.lfz` 文件第一行必须恰好 `#42`+行终止符；仅文件模式；无变体；失败抛 `CosmosAnswerError`（§2.2、§6-B、§8） |
...
- **仪式感（v0.3 新增）**：每个源码文件以 `#42` 起手——「宇宙的答案」，缺失即 `CosmosAnswerError`。
...
### 2.2 文件前导 `#42`（v0.3 核心新增）

**定义（规范性）**：

1. 前导是一行**恰好等于** `#42` 的内容（三个字符：`U+0023` `U+0034` `U+0032`），**其后必须紧跟一个行终止符**。
2. **`#` 是「文件开头专用符号」，与注释无关**：注释仍用 `//` 与 `/* */`。`#` 出现在文件起始前导位之外的任何位置（字符串 / 注释 / 格式说明符之外）→ 非法字符 `SyntaxError`。
3. **仅对「文件模式」生效**：源码来自**文件路径**时强制要求；**REPL / stdin / `lfz run -e`（内联）豁免**（§6-B9）。
4. **不允许任何变体**：`# 42`、`#42abc`、`#43`、`#42 `（尾随空格）、`#42\t`、`##42`、前导空行/空格、BOM 之后再空行——全部违规。
5. 不满足 → 抛 **`CosmosAnswerError`**，用户可见消息**恒为**「你忘记了宇宙的答案」，**不显示编号、不显示 hint**（§8）。

**加载器算法（唯一确定，逐字节）**：

```
load_file(path):
  1. 读入全部字节；若 0 字节 -> CosmosAnswerError（line 1）。
  2. 按 UTF-8 解码整文件；解码失败 -> SyntaxError（编码错误），终止。
     （先编码、后前导：非 UTF-8 文件报编码错，而非 CosmosAnswerError。）
  3. 若文本以 U+FEFF 开头 -> 去掉该字符（仅一次）。
  4. 若文本为空 -> CosmosAnswerError（line 1）。
  5. 取前 3 个字符，必须严格等于 "#42"；否则 -> CosmosAnswerError（line 1）。
  6. 第 4 个字符必须是行终止符之一：\n | \r\n | \r；
     若是 EOF 或其它字符（含空格/Tab/字母/第 2 个 '#'）-> CosmosAnswerError（line 1）。
  7. 消费该行终止符（\r\n 记作一个）；前导整行（含终止符）被消费，**不产出任何 token**。
  8. 将其余文本中的 \r\n 与单独 \r 归一化为 \n；交接给 lexer。
  9. 行号从 1 起计：前导所在行 = line 1，程序体从 line 2 开始。
load_source(text):          # REPL / stdin / -e 使用
  跳过步骤 1–7（不要求前导、不消费首行）；
  仍做步骤 8（归一化）与 2（若 text 由字节解码而来）。
```

**规则冲突消解（规范性）**：
- `#` 只在**加载器的文件前导位**（步骤 5–7）被消费；lexer 的 CODE 模式**从不接受** `#`。
- 因此 `#` 合法出现的场景**仅**：(a) 文件前导位；(b) 字符串文本内（STR 模式）；(c) 注释内；(d) 格式说明符文本内（§2.8）。其余一律 `SyntaxError`。
...
- **`#` 不是记号**：只在前导位由加载器消费，CODE 模式下出现即非法字符（§2.2）。
- **禁止合成**（不存在这些记号，必须拆开）：`>>` `<<` `++` `--` `->` `..` `??` `::` `|` `&`。
...
## 6. 新增规则的歧义 / 边界审计（B1–B12）★ v0.3 新增

> 针对用户本轮新指令（文件前导 `#42` / 错误模型 / 可移植性）逐项给出**规则 + 正例 + 反例**。默认入口模式记为 `FILE`；若涉及其它入口模式则显式标注。

### B1. 空文件（0 字节）
- **规则**：文件模式要求前导 → 空文件 → `CosmosAnswerError`（line 1）。
- 正例：无（空文件必错）。
- 反例：`build artifacts` 产生的 0 字节 `x.lfz` → `CosmosAnswerError: 你忘记了宇宙的答案`。

### B2. 只有 `#42`、无行终止符（EOF 紧随）
- **规则**：`#42` 后**必须**紧跟行终止符；EOF → `CosmosAnswerError`（line 1）。
- 正例：`#42\n`（合法，程序体为空）；`#42\r\n`、`#42\r` 均合法。
- 反例：文件内容恰为 3 字节 `#42` → `CosmosAnswerError`。
...
### B9. REPL / stdin / `lfz run -e` 豁免
- **规则**：只有**文件模式**要求前导；REPL / stdin / `-e` **豁免**（§2.2 步骤 1–7 跳过）。
- 正例：`lfz run -e 'print(1 + 2)'`、`echo 'print(1)' | lfz run -`、REPL 输入 `let x = 1` → 均**不**要求 `#42`。
- 反例：无（豁免是定义）。
- **附则**：`-e`/stdin/REPL 的出错位置伪路径分别为 `<command>` / `<stdin>` / `<repl>`（§8.3）。

### B10. `CosmosAnswerError` 的位置信息
- **规则**：无前导时，位置**固定为第 1 行**；**不显示源码行、不显示插入符、不显示编号/hint**。
- 正例（输出）：
  ```
...
### B11. `lfz init/fmt/test/check` 是否统一查头
- **统一原则（规范性）**：**"文件即需前导"（file ⇒ header）**。凡以**文件**身份进入 LFZ 源码加载器的输入，**一律**由加载器校验前导；因此**所有子命令自动一致**。
- 分命令约定：
  | 命令 | v1 状态 | 是否查头 | 失败行为 |
  |---|---|---|---|
  | `lfz run <file>` | ✅ 实现 | **是** | `CosmosAnswerError`，退出码 2 |
  | `lfz test` | ✅ 实现 | **是**（每个测试文件） | 记该用例为 error，退出码 2 |
  | `lfz check <file>` | ⏸ 延后 | **是**（规则预留） | 同 `run` |
  | `lfz fmt <file>` | ⏸ 延后 | **是**（规则预留；fmt **不**自动补前导） | `CosmosAnswerError`，退出码 2 |
  | `lfz init` | ⏸ 延后 | 生成的项目 `.lfz` 模板**自动含 `#42` 首行**（新手不踩坑） | — |
  | REPL / stdin / `-e` | ✅/⏸ | **否（豁免）** | — |
- 正例：`lfz run ok.lfz`（有头）→ 正常；`lfz test`（全部测试带头）→ 正常。
- 反例：`lfz run noheader.lfz` → `CosmosAnswerError`，退出码 2。

### B12. 前导的字节精确性与"唯一入口"
- **规则**：`#42` 校验在**解码后、词法前**完成；`#` 不进入记号流。lexer 永不产生 `#` 记号。
- 正例：`#42` 首行被消费且**不产出 `NEWLINE` 记号**（程序体首行的行号仍从 2 起）。
...
(* ============================================================
   ...
   约定:
   - 加载器（loader）在词法前完成: UTF-8 校验 -> 跳 BOM -> 校验并消费 preamble
     -> 归一化行终止符。违反 preamble 规则 -> CosmosAnswerError。
...
(* 规范性 mode 区分:
   - 文件模式 (源码来自文件路径): preamble 必须出现, 否则 CosmosAnswerError。
   - 非文件模式 (REPL / stdin / -e): preamble 必须缺失(省略)。
   加载器消费 preamble, 不产出任何记号 (含不产出 NEWLINE)。 *)
...
|---|---|---|---|
| `LfzError`（基类） | 不直接抛出 | — | — |
| `CosmosAnswerError` | 文件模式缺少合法 `#42` 前导（§2.2 步骤 1–7 任一失败） | **`你忘记了宇宙的答案`**（固定，无参数） | 加载 |
...
> **hint 行策略**：v1 用户可见输出**不含** `hint`/`提示` 行（保证黑盒测试逐字符稳定）；`CosmosAnswerError` **明确禁止**任何 hint。结构化错误（特色 4）由「类名 + 中文消息 + 位置 + traceback + `--json` 字段」满足。
...
- 新模块 `loader`（建议 `src/loader.rs`），职责（§2.2）：读文件 → UTF-8 校验（失败 → `SyntaxError`）→ 跳 BOM → 校验/消费 `#42` 前导（失败 → `CosmosAnswerError`）→ 归一化行终止符。
- 对外暴露两类入口：
  - `load_file(path) -> Result<String, LfzError>`：**要求前导**。
  - `load_source(text) -> Result<String, LfzError>`：**不要求前导**（REPL / stdin / `-e`）。
- 前导**不产出 token**；行号从 1 起，程序体自 line 2 开始。

### 10.2 AST 必须携带源码位置（新增硬要求）

- Python 风格报错要求**行列信息**，故 AST 节点**必须**携带 `span`：
  ```rust
  struct Span { line: u32 /*1-based*/, col: u32 /*1-based, Unicode 标量计数*/ }
  ```
...
## 11. 对下游角色的硬性要求（规范，供 ai-dx / test-engineer / docs / app 执行）★ v0.3 新增

### 11.1 AI 开发指南 / skill（ai-dx-engineer）

AI 指南与 `lfz-programming` skill（计划路径 `.opencode/skills/lfz-programming/`）**必须写死**以下内容：

1. **`#42` 首行规则（置顶）**：任何 `.lfz` 文件的**第一行必须恰好是 `#42`** 三个字符，其后紧跟一个行终止符；否则程序以
   `CosmosAnswerError: 你忘记了宇宙的答案` 终止。REPL / stdin / `lfz run -e` **不需要** `#42`。
...
- **T-R1**：所有被 runner 作为 LFZ 程序加载的测试文件**必须是带 `#42` 首行的合法 `.lfz` 文件**；缺失 → 该用例记为 **error**（退出码 2）。
...
- **T-R4**：runner 发现测试文件的规则须明确写入契约（建议 glob 如 `tests/**/*.lfz`，并排除 `tests/fixtures/**`）。
...
- 冻结时把本草案拆入 `docs/spec/{syntax,semantics,interface-contract}.md`（**计划路径，当前不存在**）：
  ...
  - `interface-contract.md`：§8.1 错误类 + §10 的 `Span` / `CallFrame` / `LfzError` / `DebugSym` / loader 接口。
...
1. **入口模式判定的粒度**：当前 = **"文件即需前导"**（凡从文件路径加载即要求 `#42`，**不限扩展名**；约定扩展名 `.lfz`）。可选 = **严格仅 `.lfz` 扩展名**要求前导（其它扩展名按文件处理但不校验）。影响：脚本以临时名运行时是否被拦。
2. **`CosmosAnswerError` 的源码行显示**：当前 = **不显示**源码行/插入符（仅 `File ..., line 1`）。可选 = 显示第 1 行原文（便于看出写成 `# 42` 还是空行），但会让输出对每种违规形态不同。
3. **`#42` 前导是否要求"文件至少有一行"之外的非空程序体**：当前 = 不要求（`#42`+换行即合法，程序体可空）。可选 = 要求至少一条语句。
4. **错误 `hint` 行**：当前 = v1 **不输出** hint（`CosmosAnswerError` 明确禁止）。可选 = 为 `SyntaxError` 附可选「提示：…」行。影响：黑盒测试稳定性 vs 新手友好度。
5. **（沿用 v0.2 §11）** `;;` 作用域范围（完整可见链 vs 仅当前函数）、`;;` 输出通道（stdout vs stderr）、多行续行糖（仅括号 vs 允许行尾/行首续行）、单变量打印形式（单 `;` 一律错 vs `;name`）。**v0.3 全部沿用 v0.2 默认**，仍可覆盖。
...
> v0.2 §9 的 10 条对账结论在 v0.3 **全部维持**，仅把错误码替换为类名。摘要：
...
- **#5 换行终结（ASI）**：`return` 行尾 = 返回 `nil`；**括号是唯一续行手段**（部分不同意 core-dev 的续行组合，见 §14-5）。
...
*（本文件为 v0.3 草案，待用户确认 §14 开放项后随 v1 冻结生效；冻结时走 ADR 并拆分为 `docs/spec/` 三件套。）*
